...
const Hamster = () => {
  ...
  const productosHamster = productos.filter(
    (producto) => producto.categoria?.toLowerCase() === "hámster"
  );

  const [currentPage, setCurrentPage] = useState(1);
  const productosPorPagina = 6;
  const totalPages = Math.ceil(productosHamster.length / productosPorPagina);
  const inicio = (currentPage - 1) * productosPorPagina;
  const productosMostrados = productosHamster.slice(inicio, inicio + productosPorPagina);

  return (
    <main className="min-h-screen bg-gray-50 p-8">
      <div className="text-center mb-8">
        <h1 className="text-3xl font-bold text-blue-700 mb-2">Todo para hámsters</h1>
        <p className="text-gray-700 text-lg">
          Lo mejor para tu pequeño roedor: hábitats, snacks y juguetes divertidos.
        </p>
      </div>

      {productosMostrados.length === 0 ? (
        <p className="text-center text-gray-500">No hay productos para hámsters disponibles.</p>
      ) : (
        <>
